...
export class Test {
    ...
    async init() {
        ...
        if (userInfo.testId) {
            try {
                const result = await CustomHttp.request(config.host + '/tests/' + userInfo.testId);
    ...
    async complete() {
        ...
        try {
            const result = await CustomHttp.request(config.host + '/tests/' + userInfo.testId + '/pass', 'POST', {
